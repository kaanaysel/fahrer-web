...
label{display:block;font-weight:800;margin-bottom:6px}input,select,textarea,button,.btn{width:100%;padding:11px 12px;border:1px solid #c7ceda;border-radius:12px;font-size:15px;background:#fff}textarea{min-height:42px;resize:vertical}button,.btn{cursor:pointer;text-decoration:none;text-align:center;display:inline-block;background:#f8fafc;font-weight:800}.btn.primary,button.primary{background:linear-gradient(135deg,var(--blue),var(--blue2));border-color:var(--blue);color:#fff}.btn.danger,button.danger{background:#fff1f0;border-color:#fda29b;color:#b42318}.btn.small{width:auto;padding:8px 11px;border-radius:10px;font-size:13px}.actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.actions .btn,.actions button{width:auto}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:18px;background:#fff}table{border-collapse:separate;border-spacing:0;width:100%;min-width:1180px}th,td{padding:12px;border-bottom:1px solid #edf0f5;text-align:left;vertical-align:middle}th{position:sticky;top:0;background:#f3f6fb;color:#344054;font-size:13px;z-index:1}tr:hover td{background:#fbfdff}.flash{padding:12px 14px;border-radius:14px;margin-bottom:14px;font-weight:700}.flash.ok{background:#ecfdf3;color:#067647;border:1px solid #abefc6}.flash.err{background:#fff1f0;color:#b42318;border:1px solid #fecdca}.login-wrap{max-width:520px;margin:8vh auto;padding:24px}.driver-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px}.month-card{padding:16px;border:1px solid var(--line);border-radius:18px;background:#fff;text-decoration:none}.month-card strong{display:block;color:#123e7c;font-size:1.1rem;margin-bottom:6px}.right{text-align:right}.nowrap{white-space:nowrap}.item-row{display:flex;gap:8px;align-items:center;margin-bottom:6px;padding:6px 8px;border:1px solid #edf0f5;border-radius:12px;background:#fbfdff}.item-row form{margin-left:auto}.mini-form{display:grid;grid-template-columns:82px 74px minmax(115px,1fr) minmax(118px,1fr) auto;gap:7px;align-items:center}.sum-box{font-size:13px;margin-top:8px;color:var(--muted)}.admin-info{min-width:0}.admin-info textarea{min-height:86px;font-size:14px;background:#fffef7;border-color:#f6d98b}.download-note{font-size:12px;color:var(--muted);margin-top:4px}.driver-row.row-base td{background:#ffffff}.driver-row.row-alt td{background:#f3f6fb}.driver-row:hover td{background:#eaf1fb!important}.adjustment-list{margin-top:12px;padding-top:10px;border-top:1px dashed #cfd6e3}.delete-month-btn{font-size:11px!important;padding:5px 8px!important;border-radius:9px!important;opacity:.82}.delete-month-btn:hover{opacity:1}.admin-info textarea.carried{background:#f5f8ff;border-color:#b8c8f0}.month-table-wrap{overflow-x:visible}.month-admin-table{min-width:0;table-layout:fixed}.month-admin-table th,.month-admin-table td{padding:9px 7px}.month-admin-table input,.month-admin-table select,.month-admin-table textarea{padding:9px 8px}.month-admin-table .hour-input{min-width:0;width:7.2ch}.month-admin-table .v-input{min-width:0;width:5.4ch}.month-admin-table .adjustments-cell{min-width:0}.month-admin-table .actions{gap:6px}.month-admin-table .actions .small{padding:8px 9px}.month-admin-table col.info-col{width:17%}.month-admin-table col.driver-col{width:12%}.month-admin-table col.hours-col{width:7.5%}.month-admin-table col.payroll-col{width:8%}.month-admin-table col.v-col{width:6%}.month-admin-table col.adjust-col{width:27%}.month-admin-table col.small-col{width:6%}.month-admin-table col.action-col{width:10.5%}
@media(max-width:900px){.shell{display:block}.side{position:relative;height:auto}.main{padding:14px}.grid-2,.grid-3,.grid-4{grid-template-columns:1fr}.top{display:block}.title{font-size:1.55rem}.mini-form{grid-template-columns:1fr}.month-table-wrap{overflow:visible;border:0;background:transparent}.month-admin-table,.month-admin-table tbody,.month-admin-table tr,.month-admin-table td{display:block;width:100%}.month-admin-table{min-width:0}.month-admin-table colgroup,.month-admin-table tr:first-child{display:none}.month-admin-table td{border-bottom:0;padding:8px 0}.month-admin-table tr.driver-row{border:1px solid var(--line);border-radius:18px;background:#fff;margin-bottom:14px;padding:12px;box-shadow:var(--shadow)}.month-admin-table td::before{content:attr(data-label);display:block;font-size:12px;font-weight:900;color:var(--muted);margin-bottom:5px}.month-admin-table .hour-input,.month-admin-table .v-input{width:100%}.month-admin-table .actions{display:flex}.month-admin-table .actions button{width:auto}}
"""
...
def admin_months():
    ...
    <div class="card"><h2>{{ months[month] }} {{ year }}</h2><div class="table-wrap month-table-wrap"><table class="month-admin-table"><colgroup><col class="info-col"><col class="driver-col"><col class="hours-col"><col class="payroll-col"><col class="v-col"><col class="adjust-col"><col class="small-col"><col class="small-col"><col class="small-col"><col class="action-col"></colgroup>
      ...
      <tr class="driver-row {{ 'row-alt' if loop.index0 % 2 else 'row-base' }}">
        ...
        <td data-label="Stunden"><form method="post" id="save-{{ d['id'] }}"><input type="hidden" name="action" value="save"><input type="hidden" name="driver_id" value="{{ d['id'] }}"><input class="hour-input" name="worked_hours" inputmode="decimal" value="{{ r['worked_hours'] if r else '' }}"></form></td>
        <td data-label="Abrechnung"><input class="hour-input" form="save-{{ d['id'] }}" name="payroll_hours" inputmode="decimal" value="{{ r['payroll_hours'] if r else '' }}"></td>
        <td data-label="V"><input class="v-input" form="save-{{ d['id'] }}" name="v_hours" inputmode="decimal" value="{{ r['v_hours'] if r else '' }}"></td>
