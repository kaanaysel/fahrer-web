...
def sick_days_pdf_display(raw: str) -> str:
    """PDF display: emphasize the total sick-day count in bold and underline."""
    value = str(raw or "").strip()
    if not value:
        return ""
    count = sick_days_count(value)
    return f"{value} = <b><u>{count} Tage</u></b>"
...
def export_payroll_hours_pdf(conn: sqlite3.Connection, year: int, month: int) -> Path:
    ...
    pdf_rows = [[
        ...
        color_text(sick_days_pdf_display(row_get(r, "sick_days", "")) or "-", "#b42318"),
    ] for r in rows]
    if not pdf_rows:
...
def admin_payroll_hours():
    ...
    """, year=year, month=month, months=MONATE, editable=editable, locked_note=locked_note, drivers=drivers, rows=rows, fmt_hours=fmt_hours, fmt_signed=fmt_signed, fmt_v_input=fmt_v_input, fmt_decimal_input=fmt_decimal_input, signed_class=signed_class, row_get=row_get, row_v_enabled=row_v_enabled, employment_class=employment_class, vacation_display=vacation_display, sick_days_display=sick_days_display, sick_days_count=sick_days_count, global_v_all=global_v_all)
